...
## 5차 확장 (2026-08-27) — 파인튜닝 리서치에서 확인된 용어

> 아래 4건은 raw 수집물(LLM 파인튜닝 영상 11편)에서 반복 등장했으나 용어집에 항목이 없던 것.
> arXiv ID·제1저자·연도·공식문서 문구는 **2026-08-27에 실제 페이지를 열어** 대조했다.

| 용어(원어/한글) | 검증 출처 | 한 줄 정의 | 흔한 오해 |
|---|---|---|---|
| **PEFT / 파라미터 효율 파인튜닝** | Hugging Face, *PEFT documentation*, [huggingface.co/docs/peft](https://huggingface.co/docs/peft/index) | 파라미터 대부분을 얼린 채 소수만 학습해 비용을 낮추면서 전체 파인튜닝에 준하는 성능을 노리는 기법군 | "PEFT=LoRA" 아님(LoRA는 하위 기법). 파인튜닝 일반의 한계(지식 주입 부적합)는 그대로 적용 ⚠️총칭 개념 — 유일 창시 논문 없음 |
| **SFT / 지도 파인튜닝** | Ouyang et al. **arXiv:2203.02155**(2022) | 사람이 쓴 모범 출력 쌍으로 사전학습 모델을 추가 학습하는 단계. InstructGPT의 1단계 | RLHF와 혼동 금지(SFT는 그 이전 단계). 새 지식 주입 용도로 신뢰 불가 ⚠️대표 출처이며 유일 창시 아님 |
| **Catastrophic forgetting / 파국적 망각** | Kirkpatrick et al. **arXiv:1612.00796**(2016) | 새 과제를 순차 학습할 때 이전 과제 성능이 급격히 상실되는 현상 | 과적합과 다름(과적합=새 데이터 일반화 실패 / 망각=옛 과제 상실). 해결된 문제 아님 ⚠️현상 자체는 1980년대부터 보고 — 유일 창시 아님 |
| **Tokenizer / 토크나이저** | Sennrich et al. **arXiv:1508.07909**(2015) | 원문을 어휘집 단위로 쪼개 ID로 바꾸는 구성요소. 드문 단어는 부분단어 열로 표현 | "토큰=단어" 아님. 모델과 토크나이저는 짝 — 어긋나면 조용히 망가짐 ⚠️BPE 대표 논문이며 토크나이저 일반의 창시 아님 |

---

## 6차 확장 (2026-08-30) — 비용·평가·정렬의 빈칸

> 아래 4건은 Obsidian wiki(`concepts/AI` 34개 문서)와 용어집을 대조해 드러난 공백이다.
> arXiv ID·제1저자·게재일은 **2026-08-30에 arXiv 공식 API로 조회해 대조**했고, 공식문서는 원문 문구를 인용했다.

| 용어(원어/한글) | 검증 출처 | 한 줄 정의 | 흔한 오해 |
|---|---|---|---|
| **Prompt caching / 프롬프트 캐싱** | Anthropic, *Prompt caching* 공식문서 · OpenAI, *Prompt Caching in the API* | 반복되는 프롬프트 접두부를 제공사가 저장해 재사용하는 기능. 재사용분은 싸게 과금된다 | "캐싱은 항상 이득" 아님 — **캐시 쓰기가 일반 입력보다 비싸다**(Anthropic). OpenAI는 **1,024토큰 초과**에만 적용. KV 캐시와 층위가 다름 ⚠️벤더별 구현 병존 — 유일 창시 없음. **할인율·수명은 수시 변동 → 금액 계산 전 원문 재확인** |
| **KV cache / KV 캐시** | Shazeer **arXiv:1911.02150**(2019) · Kwon et al. **arXiv:2309.06180**(2023, PagedAttention) | 디코딩 중 앞선 토큰의 키·값 텐서를 보관해 재사용하는 기법 | "모델 크기만 보면 VRAM을 안다" 아님 — 캐시는 **대화 길이에 비례해 증가**. 프롬프트 캐싱과 다름 ⚠️트랜스포머 디코딩의 표준 기법 — 유일 창시 논문 없음 |
| **LLM-as-a-judge / 엘엘엠 심판** | Zheng et al. **arXiv:2306.05685**(2023, 저자 13인) | 인간 채점 대신 강한 LLM에게 출력을 평가·비교시키는 방식 | "모델이 채점하면 객관적" 아님 — 논문 제목 자체가 *심판을 심판한다*. 위치·장황함·자기선호 편향 ⚠️평가 관행의 대표 문헌이며 유일 창시 아님. **편향 유형의 원문 표현은 초록·본문 대조 남음** |
| **Reward hacking / 보상 해킹** | Skalse et al. **arXiv:2209.13085**(2022) · Amodei et al. **arXiv:1606.06565**(2016) | 대리 지표를 최적화한 결과 지표 점수는 오르나 실제 의도한 목표에서 멀어지는 현상 | "보상을 잘 설계하면 없앨 수 있다" 아님 — 대리 지표를 쓰는 한 구조적. 과적합과 다름(평가 기준 자체를 공략) ⚠️강화학습에서 오래 관찰된 문제 — 유일 창시 아님 |

**등재하지 않고 보류한 것과 이유** — 같은 대조에서 후보로 올라왔으나 **1차 출처를 이번에 열어 확인하지 못해** 뺐다: Sandbox·Ontology(W3C OWL)·Human-in-the-loop·Context compaction·Hybrid search/BM25(Robertson & Zaragoza 2009는 arXiv 미등재)·Orchestration(AutoGen arXiv:2308.08155는 실재 확인했으나 항목 설계 미완)·Knowledge graph/GraphRAG(arXiv:2003.02320·2404.16130 실재 확인, 동일)·Model routing(arXiv:2406.18665·2404.14618 실재 확인, 동일)·Test-time compute(arXiv:2408.03314 실재 확인, 동일). **AX**는 국내 통용 조어로 표준 출처를 확인하지 못해 보류하고, **Context rot**은 5차 확장에서 기각한 사유(원 보고서 미확인)가 그대로 유지된다. 출처가 확인되면 그때 넣는다.
