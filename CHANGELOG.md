...
### Added — 프롬프트 캐싱·KV 캐시·LLM 심판·보상 해킹 (2026-08-30)

Obsidian wiki의 AI 개념 문서 34편과 용어집을 대조해 **쓰이고 있는데 정의하지 않은 말**을 찾았다. 그중 **1차 출처를 실제로 열어 확인한 4건**만 넣었다.

- **Prompt caching · 프롬프트 캐싱** — 이 용어집에서 비용에 관한 가장 큰 구멍이었다. [Token]과 [Context window]는 있는데 **실무에서 청구서가 실제로 정해지는 지점**이 없었다. 두 제공사의 설계가 다르다는 점(Anthropic은 명시 지정, OpenAI는 1,024토큰 초과 시 자동)을 함께 실어 벤더 중립으로 썼다. 「흔한 오해」의 핵심은 **캐시 쓰기가 일반 입력보다 비싸다**는 것 — 재사용이 없으면 손해다. ⚠ 할인율·수명은 제공사가 수시로 바꾸므로 금액 계산 전 원문 재확인을 항목에 명시했다.
- **KV cache · KV 캐시** — [VRAM]을 넣으면서 정작 **그것을 잡아먹는 주범**을 빠뜨렸다. "모델 크기만 보면 VRAM을 가늠할 수 있다"는 오해를 교정한다(캐시는 대화 길이에 비례해 자란다). 출처: Shazeer (2019), arXiv:1911.02150 · 운영 측면 Kwon et al. (2023), arXiv:2309.06180.
- **LLM-as-a-judge · 엘엘엠 심판** — [Evals]가 홀로 서 있었다. 평가를 돌리면 곧장 "누가 채점하나"에 부딪히는데 그 답이 없었다. 05장의 OpenPipe RULER 사례가 이미 이 방식을 쓰고 있었으므로 **사례가 참조할 정식 항목**이 생긴 셈이다. 출처: Zheng et al. (2023), arXiv:2306.05685.
- **Reward hacking · 보상 해킹** — 05장이 가장 얇았다(6항목). RLHF·DPO·GRPO가 다 있는데 **그것들이 왜 어려운지**가 없었다. 과적합과의 차이(평가 기준 자체를 공략한다)를 「흔한 오해」에 넣었다. 출처: Skalse et al. (2022), arXiv:2209.13085 · 선행 Amodei et al. (2016), arXiv:1606.06565.

전체 101 → **105개 항목**. 인용한 arXiv ID는 **2026-08-30에 arXiv 공식 API로 조회해 제목·제1저자·게재일을 대조**했다.

**넣지 않은 후보와 이유** — 대조에서 16건이 올라왔지만 12건을 뺐다. Orchestration·Knowledge graph/GraphRAG·Model routing·Test-time compute는 **논문 실재는 확인했으나** 항목 설계를 마치지 못했고, Sandbox·Ontology·Human-in-the-loop·Context compaction·Hybrid search/BM25는 **1차 출처(W3C·공식문서·비-arXiv 논문)를 이번에 열지 못했다.** **AX**는 국내에서 자주 쓰이지만(wiki 72회) 표준 출처를 찾지 못해 보류했다 — 빈도가 높다고 넣으면 용어집이 아니라 유행어집이 된다. **Context rot**은 앞서 기각한 사유(원 보고서 미확인)가 그대로다. 이 용어집의 기준은 여전히 "출처 없는 정의 = 미완성"이다.


### Changed — 내부 링크 앵커 보정 · EU AI Act 적용일 갱신 (2026-08-30)

**① 교차 참조가 실제로 그 용어에 가 닿게 했다.** 파일 안 링크의 절반(142개)이 `[Context window](01-llm-basics.md)`처럼 **앵커 없이 파일 상단으로만** 떨어지고 있었다. 101개 항목이 한 파일에 들어 있으므로, 앵커가 없으면 독자는 도착한 뒤 다시 찾아야 한다. 전량을 `#앵커`까지 붙여 **286개 교차 링크 전부**가 해당 표제어로 직행하게 했다(깨진 앵커 0건 검증).

같은 작업에서 **표시 텍스트에 파일명이 노출된 6곳**을 고쳤다 — `[09-local-run.md](09-local-run.md)`, `[Evals → 07-dev-stages.md](...)` 같은 것들로, 독자에게 파일명이 그대로 보이고 있었다.

**② TABLE.md 푸터** — 본문은 101개인데 푸터만 "총 88개"로 남아 있었다.

**③ EU AI Act 위험등급 — 적용일 체계 갱신.** v1.0 집필(2026-07-15) 직후인 **2026-07-27 'Digital Omnibus on AI'(Regulation (EU) 2026/1744)가 발효**해 AI Act의 Article 113(적용일)을 개정했다. 고위험 의무가 **Annex III 유형 2027-12-02 · Annex I 유형 2028-08-02**로 미뤄졌다. 적용일 표를 넣고, 실무에서 가장 자주 생기는 오해 셋을 「흔한 오해」에 추가했다 — **연기는 고위험에 한정**이고 GPAI(Chapter V)는 2025-08-02부터, 금지(Article 5)는 2025-02-02부터 **이미 적용 중**이라는 것, 그리고 **연기는 적용일을 미룬 것이지 의무를 없앤 것이 아니라는 것**이다.

⚠️ **확인된 것과 확인 못 한 것을 항목 안에서 갈랐다.** 고위험 적용일·옴니버스 실재·발효일은 EUR-Lex 통합본(02024R1689-20260727)과 집행위 자료로 확인했다. 반면 **Article 5 신설 호의 개별 번호, Article 50 표시의무 유예의 조항·문언, Chapter V 비개정 여부, 2025-08-02 이전 출시 GPAI의 경과규정**은 원문을 직접 열지 못해 **「확인 범위」 블록에 미확인으로 명시**했다. 법률 항목에서 "확인했다"는 표기가 실제 열람 범위를 넘어서면 그 자체가 결함이므로, 초안이 확정형으로 적었던 조항 인용 3곳을 **"확인 필요"로 낮췄다.**

**④ 동기화 스크립트 버그 수정(리포 밖 `sync-glossary.sh`).** Vault 사본 전용 진입점 노트 `AI 용어집 바이블.md`가 rsync `--delete` 대상에서 제외되지 않아 **동기화할 때마다 삭제되는** 상태였다. 그 파일이 동기화 방법을 안내하는 유일한 문서라 EXCLUDES에 추가했다.
